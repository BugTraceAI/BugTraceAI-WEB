...
export const WebSecAgent: React.FC<WebSecAgentProps> = ({
  ...
  useEffect(() => {
    ...
    // Guard against re-persisting the SAME finished response. Compare only against
    // the LAST persisted message (not the whole history) so a legitimately repeated
    // answer later in the conversation isn't silently dropped as a "duplicate".
    const lastPersisted = persistedMessages[persistedMessages.length - 1];
    const alreadyPersisted =
      lastPersisted?.role === 'assistant' && lastPersisted.content === last.content;
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    // Auto-create a session on the first send instead of silently doing nothing when the user is on
    // a bare /chat with nothing selected (currentSession is null by design there — the previous
    // guard `|| !currentSession` made the send button appear dead). We deliberately do NOT navigate
    // to /chat/:id here: navigating would fire the URL-driven loader + history-sync effect, which
    // would race the response we're about to stream. The session is still saved (it shows in the
    // sidebar for resume); multi-turn in this session keeps working since currentSession is now set.
    let session = currentSession;
    if (!session) {
      try {
        session = await createSession('websec', rawMessage.trim().slice(0, 50) || 'WebSec Agent Chat');
      } catch (err) {
        console.error('Failed to create session:', err);
        setUserInput(rawMessage); // restore the text so the user can retry
        return;
      }
    }
    if (!session) return; // defensive type-narrow
    ...
    try {
      // 1. Persist raw message to DB — target the resolved session explicitly, since a just-created
      //    session may not be reflected in currentSession's ref yet.
      await persistMessage(rawMessage, 'user', session);
  ...
  }, [userInput, isLoading, currentSession, persistMessage, onSendMessage, webBrowsing, createSession]);
  ...
  const displayMessages = useMemo(() => {
    ...
    if (
      ...
      if (lastLegacy.role === 'model') {
        // Only suppress the live streaming bubble once THIS finished response has
        // landed as the last persisted message — comparing against the whole
        // history would hide a legitimately repeated answer.
        const lastPersisted = persistedMessages[persistedMessages.length - 1];
        const alreadyInDb =
          lastPersisted?.role === 'assistant' && lastPersisted.content === lastLegacy.content;
        if (!alreadyInDb) {
