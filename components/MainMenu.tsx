...
const toolViews: { id: View; name: string; icon: React.ReactNode; color: string; requiresCli?: boolean }[] = [
  { id: View.WEB_SEC_AGENT, name: 'Chat', icon: <ChatIcon className="h-5 w-5" />, color: 'text-coral' },
  { id: View.FINISHER, name: 'AIrepeater', icon: <ArrowPathIcon className="h-5 w-5" />, color: 'text-coral' },
  { id: View.MODEL_LAB, name: 'Model Lab', icon: <BeakerIcon className="h-5 w-5" />, color: 'text-cyan-400', requiresCli: true },
        ...
        <nav className="flex flex-col flex-grow overflow-y-auto overflow-x-hidden min-h-0">
          ...
          <div className="mb-6 mt-4">
            ...
            <div className="space-y-1">
              {toolViews.map(item => <NavLink key={item.id} view={item.id} name={item.name} icon={item.icon} color={item.color} disabled={item.requiresCli && !cliConnected} requiresCli={item.requiresCli} />)}
            </div>
          </div>
