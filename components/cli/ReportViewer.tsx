...
export const ReportViewer: React.FC<ReportViewerProps> = ({ report, onRescan }) => {
  ...
  const CLI_API_URL = (import.meta.env.VITE_CLI_API_URL || '/cli-api').replace(/\/+$/, '');
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <BrandedLoader size="h-12 w-12" label="Parsing Intelligence" />
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar bg-gradient-to-b from-[#0d0d12] to-black/20">
        {reportData && <ReportContent data={reportData} scanId={report.id} />}
      </div>
    </div>
  );
};
