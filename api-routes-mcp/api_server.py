...
# Serializes the check-and-create in start_scan so two concurrent POSTs can't
# both pass the single-active-scan guard and spawn two kr subprocesses.
# asyncio.Lock (not threading) — everything runs in one event loop.
...
async def start_scan(req: ScanRequest):
    ...
    # Hold the lock across the active-scan check AND create_scan/create_task so a
    # second concurrent request observes the new scan (created here as "pending")
    # and is rejected, instead of both spawning a kr subprocess.
    async with _scan_start_lock:
        running = await _get_running_scans()
        # A freshly created scan is "pending" until its background task flips it to
        # "running" on a later loop turn — count it as active too, or the window
        # between create_scan and that flip lets a second POST slip through.
        pending = [s for s in ScanState.active_scans.values() if s.get("status") == "pending"]
        active = running + pending
        if active:
            raise HTTPException(409, detail={
                "error": "A scan is already running",
                "active_scan_id": active[0]["scan_id"],
            })
