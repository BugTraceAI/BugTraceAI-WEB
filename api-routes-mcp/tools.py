...
class ScanState:
    ...
    # Cap retained scans so the in-memory store can't grow unbounded over uptime.
    MAX_RETAINED_SCANS: int = int(os.environ.get("MAX_RETAINED_SCANS", 200))

    @classmethod
    def _prune(cls) -> None:
        """Evict oldest terminal (completed/failed/stopped) scans beyond the cap.
        Never evicts running/pending scans — those are still being polled."""
        if len(cls.active_scans) <= cls.MAX_RETAINED_SCANS:
            return
        terminal = sorted(
            sid for sid, s in cls.active_scans.items()
            if s.get("status") in ("completed", "failed", "stopped")
        )
        excess = len(cls.active_scans) - cls.MAX_RETAINED_SCANS
        for sid in terminal[:excess]:  # scan_id is monotonic → oldest first
            cls.active_scans.pop(sid, None)
...
def _parse_kr_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    Expected format (text mode) — the bracket columns are [length, words, lines],
    where length is the response body size in BYTES (NOT a word count):
    ...
    pattern = (
        r"^(?P<method>\w+)\s+"
        r"(?P<status>\d+)\s+"
        r"\[\s*(?P<length>\d+),\s*(?P<words>\d+),\s*(?P<lines>\d+)\]\s+"
        r"(?P<url>https?://\S+)"
    )
...
async def _detect_baseline(target: str) -> tuple[int, Optional[int]]:
    ...
    Uses a GET request (matching kiterunner's actual probes) instead of HEAD.
    Many servers answer HEAD with 405 or an empty body, which mis-calibrates
    both the SPA catch-all detection and the --ignore-length noise filter.
    A GET sees exactly what kr will see.
    ...
    content_length is the response body size in bytes, or None on failure.
    """
    ...
    try:
        ...
        if len(parts) >= 2:
            ...
            try:
                ...
                length = None
        logger.info(f"Baseline probe (GET) → HTTP {status}, body {length} bytes")
...
# Static prefixes probed for catch-all behaviour when the wordlist is a binary
# .kite file (text prefix extraction is impossible). Covers the common API roots.
COMMON_API_PREFIXES = ["api", "v1", "v2", "v3", "rest", "graphql", "admin", "internal"]


def _wordlist_prefixes(wordlist_path: Path, top: int = 6) -> List[str]:
    """Most common top-level path segments in a brute (TEXT) wordlist (e.g. 'api', 'v2').

    A target may serve a uniform 200 for everything under /api/* while serving a
    different page at the root, so a single root baseline cannot calibrate the
    noise filter for that prefix. Probing the busiest prefixes catches that.
    Returns [] for binary/unreadable wordlists (kite files) — caller falls back
    to COMMON_API_PREFIXES.
    """
    counts: Dict[str, int] = {}
    try:
        with open(wordlist_path, encoding="utf-8", errors="replace") as f:
            for line in f:
                seg = line.strip().lstrip("/").split("/", 1)[0]
                # Skip comments and segments with non-path chars (binary noise)
                if seg and not seg.startswith("#") and re.fullmatch(r"[A-Za-z0-9._-]+", seg):
                    counts[seg] = counts.get(seg, 0) + 1
    except Exception:  # noqa: BLE001
        return []
    return [seg for seg, _ in sorted(counts.items(), key=lambda kv: kv[1], reverse=True)[:top]]


async def _detect_catchall_lengths(target: str, prefixes: List[str]) -> tuple[List[str], bool, int]:
    """Probe the root and each given prefix with a random path.

    Returns (ignore_lengths, catchall_detected, root_status).
    Any prefix whose random path returns 2xx is a catch-all: its body length is
    collected so kr's --ignore-length can filter the flood of false positives
    (e.g. an API that answers 200 {"error":"Not found"} for every /api/* path).
    The root body length is always collected (matches prior behaviour).
    """
    probe_targets = [target] + [f"{target}/{seg}" for seg in prefixes]
    results = await asyncio.gather(*(_detect_baseline(t) for t in probe_targets))

    ignore: List[str] = []
    catchall = False
    root_status = results[0][0] if results else 0
    for idx, (status, length) in enumerate(results):
        if not length or length <= 0:
            continue
        is_catchall = 200 <= status < 300
        if idx == 0 or is_catchall:        # always keep root size; keep prefix size only if catch-all
            if str(length) not in ignore:
                ignore.append(str(length))
            if is_catchall:
                catchall = True
    return ignore, catchall, root_status
...
# Per-(status,length) sample cap during collection so a uniform flood (e.g. a
# rate-limit 429 storm) can't balloon memory; every line is still COUNTED.
SIG_SAMPLE_CAP = int(os.environ.get("NOISE_SIG_CAP", 50))
# A non-2xx (status,length) signature seen at least this many times is treated as
# uniform noise (blanket 401/403 auth wall, 3xx/5xx catch-all) and filtered out.
NOISE_MIN_COUNT = int(os.environ.get("NOISE_MIN_COUNT", 15))


async def _collect_routes(scan_id: int, process: Any) -> tuple[List[Dict[str, Any]], Counter]:
    """Stream kr stdout into parsed routes, capping samples per (status,length)
    signature to bound memory under a flood. Returns (routes, signature_counts)."""
    routes: List[Dict[str, Any]] = []
    sig_counts: Counter = Counter()
    assert process.stdout is not None
    async for raw_line in process.stdout:
        parsed = _parse_kr_line(raw_line.decode(errors="replace"))
        if not parsed:
            continue
        sig = (parsed["status"], parsed.get("length"))
        sig_counts[sig] += 1
        if sig_counts[sig] <= SIG_SAMPLE_CAP:
            routes.append(parsed)
            ScanState.update_scan(scan_id, routes=routes, total=len(routes))
    return routes, sig_counts


def _filter_response_noise(
    routes: List[Dict[str, Any]], sig_counts: Counter
) -> tuple[List[Dict[str, Any]], Optional[str], Optional[str]]:
    """Drop uniform NON-2xx noise the pre-scan baseline can't catch when it only
    surfaces under load: rate-limit 429 storms and blanket 401/403 auth walls
    (2xx catch-alls are already filtered via --ignore-length). A few real
    401/403 endpoints survive because only signatures seen NOISE_MIN_COUNT+ times
    are dropped. Returns (filtered_routes, warning, warning_detail)."""
    noisy: set = set()
    rate_limited = auth_walls = other = 0
    for (status, length), cnt in sig_counts.items():
        if status == 429:                                   # rate-limited — never a real route
            noisy.add((status, length))
            rate_limited += cnt
        elif cnt >= NOISE_MIN_COUNT and status not in (0, 404) and not (200 <= status < 300):
            noisy.add((status, length))
    ...
    filtered = [r for r in routes if (r["status"], r.get("length")) not in noisy]
    parts: List[str] = []
    if rate_limited:
        parts.append(f"{rate_limited}× HTTP 429 — target rate-limited the scan, results may be incomplete (try a slower speed)")
    if auth_walls:
        parts.append(f"{auth_walls} paths returned a uniform 401/403 — the API likely requires authentication")
    if other:
        parts.append(f"{other} paths returned a uniform non-2xx response")
    warning = "rate_limited" if rate_limited else ("auth_required" if auth_walls else "spa_catchall")
    detail = "Filtered response noise: " + "; ".join(parts) + "."
    return filtered, warning, detail
...
async def _run_scan(scan_id: int, target: str, wordlist_path: Path, speed: str = "medium", workers: int = 20, max_connections: int = 3) -> None:
    ...
    # kr prepends "/" to every wordlist path, so a trailing slash on the target
    # produces double slashes (host/base//path). Some servers 404 on "//".
    target = target.rstrip("/")

    # Detect baseline at root + common API prefixes. Kite wordlists are binary,
    # so prefixes can't be extracted from them — use a static common set.
    detected_lengths, catchall, _root_status = await _detect_catchall_lengths(target, COMMON_API_PREFIXES)
    if catchall:
        ScanState.update_scan(
            scan_id,
            warning="spa_catchall",
            warning_detail=(
                f"Catch-all detected! Target returns a uniform response for missing paths.\n"
                f"Filtering baseline sizes ({', '.join(detected_lengths)} bytes) to isolate real endpoints."
            ),
        )
        logger.warning(f"[scan:{scan_id}] Catch-all detected — filtering sizes {detected_lengths}")

    ignore_lengths = ["0"] + detected_lengths
    ...
    try:
        ...
        routes, sig_counts = await _collect_routes(scan_id, process)

        await process.wait()
        finished = datetime.now(timezone.utc).isoformat()
        routes, noise_warning, noise_detail = _filter_response_noise(routes, sig_counts)
        ...
        if noise_warning:
            update["warning"] = noise_warning
            update["warning_detail"] = noise_detail
        if process.returncode == 0:
            ScanState.update_scan(scan_id, status="completed", **update)
            logger.info(f"[scan:{scan_id}] Completed — {len(routes)} routes found")
        else:
            ScanState.update_scan(scan_id, status="failed", error=f"kr exited with code {process.returncode}", **update)
...
async def kiterunner_scan(
    ...
    running = [s for s in ScanState.active_scans.values() if s["status"] in ("running", "pending")]
    if running:
        active = running[0]
        return {
...
async def _run_brute_scan(scan_id: int, target: str, wordlist_path: Path, extensions: str, speed: str = "medium", workers: int = 20, max_connections: int = 3) -> None:
    ...
    # Detect baseline at the root AND under each busy wordlist prefix — catches
    # per-prefix catch-alls (e.g. /api/* answering 200 for every path) that a
    # single root probe would miss, flooding results with false positives.
    prefixes = _wordlist_prefixes(wordlist_path) or COMMON_API_PREFIXES
    valid_lengths, catchall, _root_status = await _detect_catchall_lengths(target, prefixes)
    if catchall:
        ScanState.update_scan(
            scan_id,
            warning="spa_catchall",
            warning_detail=(
                f"Catch-all detected! Target returns a uniform response for missing paths.\n"
                f"Filtering baseline sizes ({', '.join(valid_lengths)} bytes) to isolate real endpoints."
            ),
        )
        logger.warning(f"[scan:{scan_id}] Catch-all detected — filtering sizes {valid_lengths}")
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            ScanState.update_scan(scan_id, status="failed", error=f"kr brute exited with code {process.returncode}", **update)
