...
export interface KrRoute {
  ...
  url: string;
  status: number;
  length?: number;  // response body size in bytes (kr's first bracket column)
  words: number;
  lines: number;
}
...
const MAX_POLL_FAILURES = 8; // ~16s of sustained polling failure before surfacing an error
...
async function backendLoadScans(): Promise<KrSavedScan[] | null> {
  try {
    ...
    return (data.data?.results ?? []).map(backendToKrSavedScan);
  } catch {
    return null;
  }
}
...
export function useKiterunner(): UseKiterunnerReturn {
  ...
  const isStartingRef = useRef(false);          // synchronous double-start guard
  const pollFailuresRef = useRef(0);            // consecutive poll failures
  ...
  const saveScanHistory = useCallback(async (scan: KrSavedScan) => {
    setScanHistory(prev => {
      const next = [scan, ...prev.filter(item => item.id !== scan.id)].slice(0, MAX_SAVED_SCANS);
      writeSavedScans(next);
      return next;
    });
    const ok = await backendSaveScan(scan);
    if (!ok) {
      setWarning('Scan saved locally but failed to sync to the server.');
      setWarningDetail('Backend persistence is unavailable; this saved scan may not appear on other devices.');
    }
  }, []);

  const removeSavedScan = useCallback(async (scanId: string) => {
    setScanHistory(prev => {
      const next = prev.filter(scan => scan.id !== scanId);
      writeSavedScans(next);
      return next;
    });
    const ok = await backendDeleteScan(scanId);
    if (!ok) {
      setWarning('Scan removed locally but failed to delete from the server.');
      setWarningDetail('The scan may reappear after syncing with backend history.');
    }
  }, []);

  const clearSavedScans = useCallback(async () => {
    setScanHistory([]);
    writeSavedScans([]);
    const ok = await backendClearScans();
    if (!ok) {
      setWarning('Saved scans cleared locally but failed to clear on the server.');
      setWarningDetail('Backend history may still contain saved scans.');
    }
  }, []);
  ...
  // Full reset of the CURRENT scan back to the pristine idle state (clears results,
  // warnings and errors). Used by the "Clear" action so the input unlocks for a new scan.
  const resetCurrentScan = useCallback(() => {
    stopPolling();
    clearActiveTracking();
  ...
  const startPolling = useCallback((scanId: number) => {
    ...
    pollTimerRef.current = setInterval(async () => {
      try {
        ...
        pollFailuresRef.current = 0;  // reset on any successful poll
        ...
        // Tolerate transient blips, but don't hang on 'Scanning…' forever if the
        // backend is permanently unreachable — surface an error after N failures.
        pollFailuresRef.current += 1;
        if (pollFailuresRef.current >= MAX_POLL_FAILURES) {
          stopPolling();
          setError('Lost connection to the api-routes service while polling. The scan may still be running on the server — reload to reconnect.');
        }
      }
    }, 2000);
  }, [fetchResults, finalizeScan, hydrateActiveScan, resetStaleScan, stopPolling]);
  ...
  const recoverPreviousScan = useCallback(async () => {
    ...
    try {
      ...
      // Finished scan persisted from a previous session: don't resurface it in the
      // current-scan view — clear the stale tracking and start clean. It stays in
      // the saved history (Old Scans).
      clearActiveTracking();
      return false;
    } catch {
      return false;
    }
  }, [clearActiveTracking, fetchResults, hydrateActiveScan, startPolling]);
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // No active/running scan: entering should start CLEAN (idle). Load history
        // ONLY to populate the "Old Scans" tab — never surface a finished scan in the
        // current-scan view on entry.
  ...
  const startScan = useCallback(async (target: string, wordlist = 'api', speed = 'medium', workers = 20, maxConnections = 3) => {
    ...
    // Synchronous guard set BEFORE any await — a rapid double-click / Enter-spam
    // fires startScan twice before scanStatus/scanIdRef update (both lag behind
    // the awaits below), so the state checks alone can't block the second call.
    if (isStartingRef.current || scanStatus === 'running' || scanStatus === 'connecting' || scanIdRef.current) {
      if (scanIdRef.current) setError('A scan is already running. Please stop it before starting a new one.');
      return;
    }
    isStartingRef.current = true;

    try {
      // Reject if another client already has a scan running on the server.
      try {
  ...
  }, [hydrateActiveScan, isReady, startPolling, scanStatus]);
  ...
  const stopScan = useCallback(async () => {
    ...
    // Best-effort stop on the backend; its failure must not skip persistence.
    try {
      await fetch(`${KR_API_BASE}/api/scan/${scanId}`, { method: 'DELETE' });
    } catch (deleteError) {
      setError(`Stop request failed: ${String(deleteError)}`);
    }

    // Best-effort capture of partial results; fall back to empty on failure.
    let resultsData: KrScanResultsResponse = { routes: [], url_list: [] };
    try {
      resultsData = await fetchResults(scanId, true);
    } catch (fetchError) {
      setError(`Stopped, but failed to fetch results: ${String(fetchError)}`);
    }

    // Always finalize + persist the stopped scan, regardless of the above.
    clearActiveTracking();
