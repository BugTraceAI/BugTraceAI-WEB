...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendMessage = useCallback(async (
    ...
    // sessionOverride lets a caller target a just-created session deterministically, before the
    // currentSession state/ref has re-rendered (used by auto-create-then-send on a fresh /chat).
    const session = sessionOverride ?? currentSessionRef.current;
    if (!session) {
