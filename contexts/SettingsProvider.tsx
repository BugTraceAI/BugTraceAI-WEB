...
import { DEFAULT_CHAT_MODEL_KEY } from '../lib/curatedModels.ts';
...
interface SettingsContextType {
    ...
    // Chat — WebSec Agent tool-loop circuit breaker (max consecutive tool-call rounds)
    maxToolHops: number;
    setMaxToolHops: (n: number) => void;
}
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [openRouterModel, setOpenRouterModel] = useState<string>(DEFAULT_CHAT_MODEL_KEY);
    ...
    useEffect(() => {
        try {
            ...
            // Load chat circuit breaker
            const savedHops = localStorage.getItem('chatMaxToolHops');
            if (savedHops) {
                const n = parseInt(savedHops, 10);
    ...
    useEffect(() => {
        try { localStorage.setItem('chatMaxToolHops', String(maxToolHops)); }
        catch (e) { console.error("Could not save chatMaxToolHops:", e); }
    }, [maxToolHops]);
