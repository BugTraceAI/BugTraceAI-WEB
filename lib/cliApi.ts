...
export interface ScanSummary {
  ...
  findings_count?: number; // Compatibility alias for detections_count
  detections_count?: number;
  confirmed_count?: number;
  manual_review_count?: number;
  reportable_count?: number;
  max_depth?: number; // Crawl depth the scan is actually running with (from the CLI record)
  max_urls?: number; // URL cap the scan is actually running with (from the CLI record)
  scan_type?: string; // Scan type the running scan was launched with
}
...
export interface RepeaterFindingRequest {
  type: string;
  severity: string;
  url: string;
  parameter?: string;
...
export interface RepeaterFindingResponse {
  finding_id: number;
  created: boolean;
  success: boolean;
  message: string;
  report_refresh?: string;
}

/**
 * Persist a confirmed finding from the AI Repeater to the CLI.
 */
export async function createRepeaterFinding(
  scanId: number,
  finding: RepeaterFindingRequest,
): Promise<RepeaterFindingResponse> {
  const response = await fetch(`${CLI_API_BASE}/scans/${scanId}/findings`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(finding),
    signal: AbortSignal.timeout(CLI_FETCH_TIMEOUT),
  });
  return handleResponse<RepeaterFindingResponse>(response);
}
...
// ============================================================
// OOB Collaborator (Burp-Collaborator-style on-demand OOB server)
// ============================================================

export interface OobSessionResponse {
  session_id: string;
  domain: string;
  server: string;
...
export interface OobInteractionItem {
  protocol: string;
  full_id: string;
...
export interface OobInteractionsResponse {
  session_id: string;
  domain: string;
  interaction_count: number;
  interactions: OobInteractionItem[];
}

/** Start a new OOB collaborator session; returns the callback domain to embed in payloads. */
export async function oobStart(server?: string): Promise<OobSessionResponse> {
  const response = await fetch(`${CLI_API_BASE}/oob/start`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(server ? { server } : {}),
    signal: AbortSignal.timeout(CLI_FETCH_TIMEOUT),
  });
  return handleResponse<OobSessionResponse>(response);
}

/** Fetch interactions captured by an OOB session (keeps the session alive). */
export async function oobGetInteractions(sessionId: string): Promise<OobInteractionsResponse> {
  const response = await fetch(`${CLI_API_BASE}/oob/${sessionId}`, {
    signal: AbortSignal.timeout(CLI_FETCH_TIMEOUT),
  });
  return handleResponse<OobInteractionsResponse>(response);
}

/** Stop an OOB session and deregister it from the interactsh server. */
export async function oobStop(sessionId: string): Promise<{ stopped: boolean; session_id: string }> {
  const response = await fetch(`${CLI_API_BASE}/oob/${sessionId}/stop`, {
    method: 'POST',
    signal: AbortSignal.timeout(CLI_FETCH_TIMEOUT),
  });
  return handleResponse<{ stopped: boolean; session_id: string }>(response);
}
